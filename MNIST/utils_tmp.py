...
def neuron_selection(dataset, model, model_layer_times, model_layer_value, neuron_select_strategy,
    ...
    if '4' in list(neuron_select_strategy):
        ...
        # model_layer_relvance_top_k_tmp = [i for num, i in enumerate(model_layer_relvance_top_k_tmp) if
        #                                   num not in num_neuron4]
        # print(len(model_layer_relvance_top_k_tmp))

    # if '4' in list(neuron_select_strategy):
    #     k = 0.2 #Model4 0.3, Model1 k=0.2
    #     print("k is" + str(k))
    #     top_k = math.floor(k * len(model_layer_times))  # number of neurons to be selected within
    #
    #     global model_layer_relvance_top_k, temp, p ### 每次迭代可以取到重复的神经元，如何修改
    #     if len(model_layer_relvance_top_k) == 0:
    #         model_layer_relvance_top_k,p = lrp_neurons(dataset,model, model_name, x_test, y_test, top_k, Class)
    #         temp = model_layer_relvance_top_k.copy()
    #
    #     tmp = model_layer_relvance_top_k
    #     print(len(temp))
    #     ##replace:True表示可以取相同数字，False表示不可以取相同数字
    #     for i in range(neuron_to_cover_num):
    #         if temp:
    #             layer_name, index = random.choice(temp)
    #             temp.remove((layer_name, index))
    #
    #         else:
    #             layer_name, index = random.choice(model_layer_relvance_top_k)
    #         loss4_neuron = K.mean(model.get_layer(layer_name).output[..., index])
    #         loss_neuron.append(loss4_neuron)
