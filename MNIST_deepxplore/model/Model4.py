...
os.environ["CUDA_VISIBLE_DEVICES"] = "4"  # 指定所用的gpu
...
def Model4(input_tensor=None, train=False):
    ...
    if train:
        ...
        # save model
        model.save_weights(os.path.join(os.path.dirname(__file__), 'Model4.h5'))
        ...
        model.compile(loss='categorical_crossentropy', optimizer='adadelta', metrics=['accuracy'])
        model.load_weights(os.path.join(os.path.dirname(__file__), 'Model4.h5'))
        # score = model.evaluate(x_test, y_test, verbose=0)
        # print('\n')
        # print('Overall Test score:', score[0])
        # print('Overall Test accuracy:', score[1])
