...
import os, sys
print("ff"+str(os.path.realpath(__file__)))
print("ff1"+str(os.path.abspath(__file__)))
print("ff2"+str(os.path.dirname(__file__)))
print(os.getcwd())
os.path.join(os.path.dirname(__file__), 'test.py')
print(os.path.exists(os.path.join(os.path.dirname(__file__), 'test.py')))
...
def filter_correct_classifications(dataset, model, X, Y, Class=True):
    X_corr = []
    Y_corr = []
    X_misc = []
    Y_misc = []
    predict_times = 10 #用于带有dropout层的回归模型多次预测
    if Class:
        ...
        for i in range(len(rank_lst)):
            if abs(mean[rank_lst[i]] - Y[rank_lst[i]]) > 0.2 or \
                    (((mean[rank_lst[i]] > 0 and Y[rank_lst[i]] < 0) or (
                            mean[rank_lst[i]] < 0 and Y[rank_lst[i]] > 0)) and
                     (abs(mean[rank_lst[i]] - Y[rank_lst[i]]) > 0.2)):
                X_misc.append(X[rank_lst[i]])
                Y_misc.append(Y[rank_lst[i]])
            else:
                X_corr.append(X[rank_lst[i]])
                Y_corr.append(Y[rank_lst[i]])

    return np.array(X_corr), np.array(Y_corr), np.array(X_misc), np.array(Y_misc)

def filter_correct_classifications_old(dataset, model, X, Y, Class=True):
    X_corr = []
    Y_corr = []
    X_misc = []
    Y_misc = []
    ...
    if Class:
        ...
        act_layers = model.predict(X)
        metrics = np.sum(act_layers ** 2, axis=1)
        rank_lst = np.argsort(-metrics)

        X_misc = X[rank_lst[0:500]]
        Y_misc = Y[rank_lst[0:500]]
    ...
    for idx, layer in enumerate(model.layers):
        try:
            if 'input' not in layer.name and 'softmax' not in layer.name and \
                ...
                # 这个函数返回包含 NumPy 数组的列表。第一个数组为层的权重，第二个数组为层的偏差。
