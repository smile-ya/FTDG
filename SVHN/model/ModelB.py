...
def ModelB(input_tensor=None, train=False):
    ...
    if train:
        ...
        model.save_weights(os.path.join(os.path.dirname(__file__), 'ModelB.h5'))
        ...
        sgd = keras.optimizers.SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True)
        model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
        model.load_weights(os.path.join(os.path.dirname(__file__), 'ModelB.h5'))
