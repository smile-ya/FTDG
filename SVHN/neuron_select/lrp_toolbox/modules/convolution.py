...
class Convolution(Module):
    ...
    def __init__(self, filtersize=(5,5,3,32), padding='same',stride = (2,2)):
        '''
    ...
    def forward(self,X,lrp_aware=False):
        ...
        #padding=True #https://blog.csdn.net/wuzqchom/article/details/74785643
        # if padding:
        ...
        if self.padding=='same':
            padding_num = (wf - 1) // 2
            X = np.pad(X, ((0,0), (padding_num,padding_num), (padding_num,padding_num), (0,0)), 'constant') # FOR LENET MODELS
            self.padding_num = padding_num

        self.X = X

        Hout = (H + 2*padding_num - hf) // hstride + 1
        Wout = (W + 2*padding_num - wf) // wstride + 1
        ...
        if self.lrp_aware:
            ...
            self.Z = np.zeros((N, Hout, Wout, hf, wf, df, nf))
            for i in range(Hout):
                for j in range(Wout):
                    #修改到这里
                    self.Z[:, i, j, ...] = self.W[na, ...] * self.X[:, i * hstride:i * hstride + hf,
                                                             j * wstride:j * wstride + wf, :, na]  # N, hf, wf, df, nf
                    self.Y[:, i, j, :] = self.Z[:, i, j, ...].sum(axis=(1, 2, 3)) + self.B

                    self.Y[:,i,j,:] = np.tensordot(X[:, i*hstride:i*hstride+hf: , j*wstride:j*wstride+wf: , : ],self.W,axes = ([1,2,3],[0,1,2])) + self.B
                    # self.Y[:, i, j, :] = np.tensordot(X[:, i * hstride:, j * wstride:, :], self.W,axes=([1, 2, 3], [0, 1, 2])) + self.B
        # print(self.W[na, ...].shape)
        # print(self.X[:, 1 * hstride:1 * hstride + hf, 1 * wstride:1* wstride + wf, :, na].shape)
        # print('convolution')
        return self.Y
    ...
    def _simple_lrp(self,R):
        ...
        if self.padding == 'same':
            ...
            # Rx = Rx[:, 1:-1, 1:-1, :] #padding为same时需要执行这句话
                Rx = Rx[:, self.padding_num:-self.padding_num, self.padding_num:-self.padding_num, :]  # padding为same时需要执行这句话
        return Rx
