...
def ModelA(input_tensor=None, train=False):
    ...
    if train:
        ...
        # trainig
        model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=batch_size, epochs=nb_epoch, verbose=1,shuffle=True)
        # save model
        model.save_weights(os.path.join(os.path.dirname(__file__), 'ModelA.h5'))
        ...
        sgd = keras.optimizers.SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True)
        model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
        model.load_weights(os.path.join(os.path.dirname(__file__), 'ModelA.h5'))
