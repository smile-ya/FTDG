...
Model from below paper All-CNN-C
...
def ModelC(input_tensor=None, train=False):
    ...
    # Model definition
    x = Conv2D(96, (3, 3), padding='same', input_shape=(32, 32, 3))(input_tensor)
    x = Activation('relu')(x)
    ...
    if train:
        ...
        # save model
        model.save_weights(os.path.join(os.path.dirname(__file__), 'ModelC.h5'))
        ...
        sgd = keras.optimizers.SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True)
        model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
        model.load_weights(os.path.join(os.path.dirname(__file__), 'ModelC.h5'))
