...
import pickle
import numpy as np
from .modules import Sequential,Linear,Tanh,Rect,SoftMax,Convolution,Flatten,SumPool,MaxPool,GlobalAveragePooling2D
#Team DeepFault
        ...
        with open(path,'r') as f:
            ...
            while len(line) > 0:
                if line.startswith(Linear.__name__): # @UndefinedVariable import error suppression for PyDev users
                    ...
                    _,h,w,d,n,s0,s1,p = line.split()
                    h = int(h); w = int(w); d = int(d); n = int(n); s0 = int(s0); s1 = int(s1)
                    layer = Convolution(filtersize=(h,w,d,n), padding=p, stride=(s0,s1))
                ...
                elif line.startswith(GlobalAveragePooling2D.__name__): # @UndefinedVariable import error suppression for PyDev users
                    modules.append(GlobalAveragePooling2D()) ; c+= 1
                else:
...
def _write_keras_txt(model, destination_path, num_channels):
    ...
    from neuron_select.utils import load_dave_model
    ...
    txtfile = open(destination_path + '.txt', 'w')
    ...
    for layer, json_elem in zip(model.layers, json_obj['config']['layers']):
        if json_elem['class_name'] == 'Activation':
            ...
            # print('Linear ' + str(prev_layer.output_shape[-1]) + ' ' \
            ...
            # print(layer.output_shape)
            # print('Convolution ' + str(json_elem['config']['kernel_size'][0])\
            ...
            txtfile.write('Convolution ' + str(json_elem['config']['kernel_size'][0])\
                ...
                + str(json_elem['config']['strides'][0]) + ' ' \
                + str(json_elem['config']['strides'][1]) + ' ' \
                + str(json_elem['config']['padding']))
        elif json_elem['class_name'] == 'MaxPooling2D':
            # print(layer.output_shape)
            # print('MaxPool ' +  str(json_elem['config']['pool_size'][0]) + ' '\
            ...
            # print(layer.output_shape)
            # print(json_elem['class_name'])

            txtfile.write(json_elem['class_name'])
        elif json_elem['class_name'] == 'GlobalAveragePooling2D':
            txtfile.write(json_elem['class_name'])
        else:
